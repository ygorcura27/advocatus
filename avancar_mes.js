...
// ── Configuração — ajuste após o beta ──
const COOLDOWN_HORAS  = 0;    // 0 = sem cooldown durante testes. Mude para 4, 8, etc depois
const ENERGIA_MIN     = 20;   // energia mínima para forçar avanço sem custo extra
...
exports.avancarMes = onCall({ region: 'southamerica-east1' }, async (request) => {
  ...
  if (energiaRestante > ENERGIA_MIN) {
    throw new HttpsError('failed-precondition',
      `Você ainda tem ${energiaRestante} de energia. Use mais ações antes de avançar o mês.`);
  }

  // ── Verificar cooldown ──
  if (COOLDOWN_HORAS > 0 && j.ultimo_avanco) {
    const diff = (Date.now() - new Date(j.ultimo_avanco).getTime()) / 3600000;
    if (diff < COOLDOWN_HORAS) {
      const restante = Math.ceil(COOLDOWN_HORAS - diff);
      throw new HttpsError('resource-exhausted',
        `Você precisa descansar. Volte em ${restante}h.`);
    }
  }
  ...
  // ── 13. Janeiro: bônus anual + recesso ──
  if (isJaneiro) {
