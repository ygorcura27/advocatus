...
// Rep por vitória: mais difícil no topo (fator reduzido + curva mais íngreme)
// Próximo do cap (>80%): ganho mínimo de 1
// Longe do cap (<40%): ganho até 3
function calcGanhoRep(repAtual, cap) {
  const pct   = repAtual / cap;
  const fator = pct > 0.8 ? 0.04 : pct > 0.6 ? 0.06 : 0.08;
  return Math.max(1, Math.floor((cap - repAtual) * fator));
}
// Rep por derrota: mais punitivo no topo (difícil manter)
// Próximo do cap (>80%): perde proporcionalmente mais
function calcPerdaRep(repAtual, cap) {
  const pct   = repAtual / (cap || 35);
  const fator = pct > 0.8 ? 0.07 : pct > 0.5 ? 0.05 : 0.03;
  return Math.max(1, Math.floor(repAtual * fator));
}
...
exports.processarSentenca = onCall({ region: 'southamerica-east1' }, async (request) => {
  ...
  if (ganhou) {
    ...
    const perdaRep = calcPerdaRep(rep, cap);
