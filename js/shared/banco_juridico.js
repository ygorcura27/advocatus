...
// (função seletora impura removida — ver estrategiasRecursoPara() no final
// deste arquivo, versão pura que recebe quemRecorre como parâmetro)
...
// Funções PURAS (recebem quemRecorre como parâmetro, nunca leem estado
// global) — usadas tanto pelo frontend quanto pela Cloud Function.
...
// ════════════════════════════════════════════════════════
// EXPORTS — API pública deste módulo, consumida pelo frontend
// (processos.js) e espelhada (mesmo conteúdo, sintaxe CommonJS) pela
// Cloud Function em functions/shared/banco_juridico.js.
//
// IMPORTANTE: qualquer mudança de CONTEÚDO jurídico (teses, provas,
// pesos, julgadores, argumentos) feita aqui precisa ser replicada
// manualmente no arquivo equivalente em functions/shared/banco_juridico.js
// — os dois arquivos não compartilham um único módulo fisicamente porque
// Cloud Functions Node.js usa CommonJS e este frontend usa ES Modules, e
// o projeto não tem pipeline de build para unificar os dois formatos.
// ════════════════════════════════════════════════════════
export {
  gerarProcesso,
  gerarTextoLocal,
  processarEvento,
  calcularPrazosRecurso,
  tribunalRecursal,
  ehTopoDaCadeia,
  classificarSentenca,
  decidirRecurso,
  ...
  gerarBancoJulgador,
  pesoTemaPorClasse,
  HINT_CLASSE_JULGADOR,
  CLASSES_JULGADOR,
  argsRecursoPara,
  respsRecursoPara,
  estrategiasRecursoPara,
  mesLabel,
  somarMeses,
  BANCO,
  TESES_TRIBUTARIO_EXPANDIDO,
};
