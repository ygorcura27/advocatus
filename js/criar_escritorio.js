...
exports.distribuirLucros = onCall({ region: 'southamerica-east1' }, async (request) => {
  ...
  // Apenas sócios podem solicitar distribuição
  const socios = esc.socios || [];
  const ehSocio = socios.some(s => s.uid === uid) || esc.fundador_uid === uid;
  if (!ehSocio) {
  ...
  // Lista de sócios com suas cotas (fallback: 100% pro fundador se array vazio)
  const listaSocios = socios.length > 0
    ? socios
    : [{ uid: esc.fundador_uid, participacao_pct: 100 }];
